...
for i in range(20):
    ...
    coords[1].append((math.sin(now), math.cos(now), 0))
    coords[2].append((math.sin(now + dist), math.cos(now + dist), 0))
    coords[3].append((math.sin(now + 2 * dist), math.cos(now + 2 * dist), 0))
    now += step
